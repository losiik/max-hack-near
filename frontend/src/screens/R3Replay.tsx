...
export function R3Replay({ id }: { id: string }) {
  ...
  const recordingUrl = loadedData.recording?.url ? `${loadedData.recording.url}${loadedData.recording.url.includes('?') ? '&' : '?'}token=${encodeURIComponent(getAccessToken())}` : null;
