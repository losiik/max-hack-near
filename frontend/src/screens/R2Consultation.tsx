...
export function R2Consultation({ id, onReplay }: { id: string; onReplay: () => void }) {
  ...
  if (detail.error || !detail.data) return <div className="notice notice--error">{detail.error instanceof Error ? detail.error.message : 'Не удалось открыть консультацию'}</div>;
